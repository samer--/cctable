...
state in Prolog requires a certain amount of term copying, and in this case,
...
efficiency, which drives us to \emph{factorise} the mutable state across several
points of mutation so that one component of the state can be modified without
...
producer results in an $O(N_p^2 K_p)$ cost overall, where $K_p$ is the size
of the data structure associated with each producer, which may be $O(1)$, $O(R_c)$,
$O(R_s)$ or $O(R_c R_s)$ depending on the representation and factorisation chosen.

\begin{tabular}{ll}
  \emph{operation} & \emph{cost} \\
  \hline
  copying each continuation before calling    & $N_p N_s (1 + R_c)$ \\
  copying each worker to get variant class    & $N_p(1 + R_c)$ \\
  copying all producers to add a new producer & $N_p^2 K_p $ \\
  % copying producer's worker to avoid binding answer & $N_p$ \\
  copying all solutions to add a consumer     & $N_p R_c R_s$ \\
  copying all continuations to add a solution & $N_p R_c R_s$ \\
  copying all continuations to add a consumer & $N_p R_c^2$ \\
  copying all solutions to add a solution     & $N_p R_s^2$ \\
\end{tabular}
